...
gulp.task('copyStaticJPG', function () {
    return gulp.src('src/static/jpg/*.jpg')
...
gulp.task('copyStaticJS', function () {
    ...
    var pugTask = FOLDERS.map(function(FOLDERS) {
        ...
        return gulp.src(path.join(SRC_PATH, FOLDERS, '/static/js/*.js'))
           .pipe(gulpCopy(DIST_PATH + '/' + FOLDERS, {
            prefix: 3,
           }));
...
// Tasks
gulp.task('build', ['images', 'templates', 'sass', 'scripts', 'processHtml', 'copyStaticJPG', 'copyStaticJS'], function() {});

gulp.task('scaffold', ['images', 'templates', 'sass', 'scripts', 'processHtml', 'copyStaticJS'], function() {});
